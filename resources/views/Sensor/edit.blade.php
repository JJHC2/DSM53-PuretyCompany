    ...
    <div class="row">
      <div class="col-md-12">
        <div class="card">
          ...
          <form action="{{ route('sensor.update',$sensor->id) }}" id="editar" method="POST" enctype="multipart/form-data"> 
          {!! csrf_field() !!}
          @method("PATCH")
          <div class="form-floating mb-3">
              <input class="form-control" type="text" value="{{$sensor->id}}" aria-label="Disabled input example" disabled/>
              <label for="">ID</label>
              </div>
            <div class="form-floating mb-3">
              <input type="number" class="form-control" id="nombre" value="{{$sensor->nombre}}" name="nombre" required>
          ...
          <div class="form-floating mb-3">
            <select class="form-control form-select" aria-label="Default select example" name="deposito_id">
                <option selected>Elige el Deposito</option>
                    ...
                    <option value="{{ $depo->id }}" @if($depo->id == $sensor->deposito_id) selected @endif>{{ $depo->codigo }}</option>
                   @endforeach
                </select>
